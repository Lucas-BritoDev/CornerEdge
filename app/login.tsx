...
export default function LoginScreen() {
    ...
    const handleLogin = async () => {
        ...
        if (success) {
            ...
            Alert.alert(t('common.error'), t('auth.login_error_invalid'));
        }
    };
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.container}>
                <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 40, paddingBottom: insets.bottom + 24 }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                    <View style={[styles.card, { backgroundColor: colors.backgroundSecondary }]}>
                        ...
                        <View style={styles.form}>
                            ...
                            <View style={[styles.inputContainer, { backgroundColor: colors.background, borderColor: colors.cardBorder }]}>
                                ...
                                <TextInput style={[styles.input, { color: colors.textPrimary }]} placeholder={t('auth.password')} placeholderTextColor={colors.textMuted} value={password} onChangeText={setPassword} secureTextEntry={!showPassword} />
                            ...
                            <TouchableOpacity style={[styles.button, { backgroundColor: colors.primary }]} onPress={handleLogin} disabled={loading} activeOpacity={0.8}>
                                <Text style={styles.buttonText}>{loading ? t('common.loading') : t('auth.login').toUpperCase()}</Text>
                    ...
                    <View style={styles.footer}>
                        <Text style={[styles.footerText, { color: colors.textPrimary }]}>{t('auth.no_account')}</Text>
                        <TouchableOpacity onPress={() => router.push('/signup')}>
                            <Text style={[styles.footerLink, { color: colors.primary }]}>{t('auth.create_account')}</Text>
                        </TouchableOpacity>
                    </View>
                </ScrollView>
